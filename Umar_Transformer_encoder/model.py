...
"""
https://www.youtube.com/watch?v=ISNdQcPhsts&t=2231s

old transformer with encoder
...
class EncoderBlock(nn.Module):
    """
    the old transformer encoder part:
        embedding -> Nx MHA, add & norm -> ffn -> add & norm -> decoder
    """
    def __init__(self, self_attention_block: MHA, feed_forward_block: FeedForwardBlock, dropout:float) -> None:
        super().__init__()
...
class DecoderBlock(nn.Module):
    """
    decoder part:
        MHA -> norm -> (+) skip 1 -> norm -> MHA -> norm -> (+) skip 2 -> FFN -> norm -> (+) skip 3
    skip 1                             skip 2          skip 3

    not the project part yet: linear -> softmax
    """

    def __init__(self,
        self_attention_block: MHA,
        cross_attention_block: MHA,
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        '''
        src_mask: applied to encoder
        tgt_mask: applied to decoder
        '''
...
class ProjectionLayer(nn.Module):
    """
    the last part: Linear -> Softmax

    d_model -> vocab size of probs
    """
...
def build_transformer(
    ...
    encoder_blocks = []
    for _ in range(N):
        encoder_self_attention_block = MHA(d_model, h, dropout)
    ...
    decoder_blocks = []
    for _ in range(N):
        decoder_self_attention_block = MHA(d_model, h, dropout)
        decoder_cross_attention_block = MHA(d_model, h, dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
