...
def get_ds(config):
    ...
    print(f"max len of tgt: {max_len_tgt}")
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            decoder_output = model.decode(encoder_output, encoder_mask, decoder_input, decoder_mask) # (b, s, d)
            proj_output = model.project(decoder_output) # (b, s, vocab)

            label = batch['label'].to(device) # (b, s)

            # (b, s, vocab size) -> (b*s, vocab size)
